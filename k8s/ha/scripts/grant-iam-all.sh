...
echo "  - Create two Cloudflare tokens in dash.cloudflare.com → My Profile → API Tokens:"
echo "      Token A  cert-manager-dns01  (Zone:DNS:Edit + Zone:Zone:Read, cloudless.gr)"
echo "      Token B  gh-actions-dns-lb   (Zone:DNS:Edit + Zone:Zone:Read + Zone:LB:Edit, cloudless.gr)"
echo "  - Then run: bash k8s/ha/scripts/bootstrap-rotation.sh --token-a A --token-b B [options]"
