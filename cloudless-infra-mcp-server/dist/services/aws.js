...
import { Route53Client, GetHealthCheckStatusCommand, GetHealthCheckCommand, } from "@aws-sdk/client-route-53";
...
let _r53 = null;
let _ssm = null;
export function getCwlClient() {
    return (_cwl ??= new CloudWatchLogsClient({ region: AWS_REGION }));
}
export function getR53Client() {
...
export async function getLambdaLogs(logGroupName, startMs, limit, filterPattern) {
...
export async function listLambdaLogGroups(prefix) {
    const client = getCwlClient();
    const res = await client.send(new DescribeLogGroupsCommand({ logGroupNamePrefix: prefix, limit: 20 }));
    return (res.logGroups ?? []).map((g) => g.logGroupName ?? "").filter(Boolean);
}
export async function getHealthCheckStatus(healthCheckId) {
    const client = getR53Client();
    const statusRes = await client.send(new GetHealthCheckStatusCommand({ HealthCheckId: healthCheckId }));
    const checkRes = await client.send(new GetHealthCheckCommand({ HealthCheckId: healthCheckId }));
    ...
    const passing = checkedRegions.filter((r) => r.status.toLowerCase().includes("success")).length;
...
export async function listSsmParameters(prefix) {
...
export async function getSsmParameter(name, decrypt = false) {
    const client = getSsmClient();
    try {
        ...
        return {
            name: p.Name ?? name,
            value: p.Type === "SecureString" && !decrypt
