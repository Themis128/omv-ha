...
import { SSH_CONFIG } from "../constants.js";
/**
 * Tries multiple common private key paths and returns the first that exists.
 */
function resolvePrivateKey(configuredPath) {
    const candidates = [
        configuredPath,
    ...
    throw new Error(`No SSH private key found. Tried: ${candidates.join(", ")}. ` +
 ...
 */
export async function runOnNode(node, command) {
 ...
 */
export async function runOnBothNodes(command) {
    const [haResult, mainResult] = await Promise.allSettled([
        runOnNode("omv-ha", command),
        runOnNode("omv-main", command),
    ]);
    return {
        "omv-ha": haResult.status === "fulfilled"
            ? haResult.value
            : { stdout: "", stderr: haResult.reason?.message ?? "Failed", code: 1 },
        "omv-main": mainResult.status === "fulfilled"
            ? mainResult.value
            : { stdout: "", stderr: mainResult.reason?.message ?? "Failed", code: 1 },
    };
}
/**
 * Uploads a local file to the remote node via SFTP.
 */
export async function uploadFile(node, localPath, remotePath) {
    const cfg = SSH_CONFIG[node];
    const ssh = new NodeSSH();
    try {
        await ssh.connect({
