...
export function registerEsp32Tools(server) {
    // ── esp32_alert_status ────────────────────────────────────────────────────
    server.registerTool("esp32_alert_status", {
...
Use to check if any cluster or dependency issues are currently firing.`,
        ...
        for (const a of alerts) {
            ...
            if (a.auto_fix)
                lines.push(`   auto-fix: ${a.auto_fix}`);
            lines.push("");
        }
        return { content: [{ type: "text", text: lines.join("\n") }] };
    });
    // ── esp32_alert_resolve ───────────────────────────────────────────────────
    server.registerTool("esp32_alert_resolve", {
...
Common codes: OMV_MAIN_K3S_API_DOWN, OMV_HA_K3S_AGENT_DOWN, CLOUDLESS_ONLINE_DOWN, OMV_MAIN_NFS_DOWN.`,
        inputSchema: z.object({
            code: z
                .string()
                .min(1)
                .describe("Alert code to resolve (e.g. OMV_MAIN_K3S_API_DOWN, CLOUDLESS_ONLINE_DOWN)"),
            resolved_by: z
                .string()
        ...
        try {
            const res = await fetch(`${ALERT_API}/api/alerts/${encodeURIComponent(code)}/resolve?resolved_by=${encodeURIComponent(resolved_by)}`, { method: "PATCH", signal: AbortSignal.timeout(8000) });
    ...
    // ── esp32_alert_history ───────────────────────────────────────────────────
    server.registerTool("esp32_alert_history", {
...
Useful for post-incident analysis and identifying flapping services.`,
    ...
    // ── esp32_device_status ───────────────────────────────────────────────────
    server.registerTool("esp32_device_status", {
...
RSSI guide: ≥ −65 dBm 🟢 good / ≥ −80 dBm 🟡 marginal / < −80 dBm 🔴 poor.`,
        ...
        const lines = ["## ESP32 Watchdog Hardware Status", ""];
