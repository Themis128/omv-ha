...
export function registerMetabaseTools(server) {
    // ── metabase_check_health ─────────────────────────────────────────────────
    server.registerTool("metabase_check_health", {
...
Note: Metabase takes ~4.5 minutes to become ready after startup (initialDelaySeconds=240).`,
        ...
        let cmd = `${KUBECTL} get pods -n ${NS} -o wide` +
    ...
    // ── metabase_h2_query ─────────────────────────────────────────────────────
    server.registerTool("metabase_h2_query", {
...
Example queries:
  ...
  SELECT id, name, engine FROM metabase_database;`,
        ...
        if (scale_down_first) {
            ...
            const scaleDown = await runOnNode("omv-main", `${KUBECTL} scale ${MB_DEPLOY} -n ${NS} --replicas=0 && ` +
        ...
        const queryCmd = `${KUBECTL} run metabase-h2-query-$RANDOM --rm -i --restart=Never ` +
            `-n ${NS} ` +
        ...
        if (qr.stderr)
            lines.push("STDERR: " + qr.stderr);
        if (qr.error)
            lines.push("ERROR: " + qr.error);
        if (scale_down_first) {
            lines.push("=== Scaling Metabase back to 1 ===");
            const scaleUp = await runOnNode("omv-main", `${KUBECTL} scale ${MB_DEPLOY} -n ${NS} --replicas=1`);
...
Standard password: TH!123789th!`,
        ...
        const waitResult = await runOnNode("omv-main", `${KUBECTL} wait job/${jobName} -n ${NS} --for=condition=complete --timeout=120s 2>&1 || ` +
            `${KUBECTL} wait job/${jobName} -n ${NS} --for=condition=failed --timeout=5s 2>&1 || true`);
        lines.push(waitResult.stdout || waitResult.error || "");
        // Get job logs
        lines.push("\n=== Job logs ===");
        const logsResult = await runOnNode("omv-main", `${KUBECTL} logs job/${jobName} -n ${NS} 2>&1 || true`);
...
The lock occurs because the previous Metabase pod held the lock; when killed, PID 0 remains in the lock record.`,
        ...
        const r1 = await runOnNode("omv-main", `${KUBECTL} rollout restart ${DUCKDB_DEPLOY} -n ${NS} && ` +
            `${KUBECTL} rollout status ${DUCKDB_DEPLOY} -n ${NS} --timeout=60s`);
        lines.push(r1.stdout || r1.error || "done");
        lines.push("\n=== Step 2: Restart Metabase ===");
        const r2 = await runOnNode("omv-main", `${KUBECTL} rollout restart ${MB_DEPLOY} -n ${NS}`);
