...
Examples: "up", "kube_pod_status_phase{namespace='default'}", "node_memory_MemAvailable_bytes"`,
        ...
        const lines = [`## Prometheus Query: \`${query}\``, ``, `**Result type:** ${resultType}`, `**Series count:** ${results.length}`, ``];
    ...
    server.registerTool("prometheus_check_targets", {
...
Returns a markdown table: job | endpoint | state | last scrape | error.`,
    ...
    server.registerTool("prometheus_check_alerts", {
...
Includes alert name, labels, and the time it started firing.`,
        ...
        const lines = [`## Prometheus Firing Alerts`, ``, `**Total firing:** ${alerts.length}`, ``];
    ...
    server.registerTool("prometheus_check_rules", {
