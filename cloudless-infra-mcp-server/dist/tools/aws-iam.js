...
export function registerAwsIamTools(server) {
    // ── aws_list_iam_users ────────────────────────────────────────────────────
    server.registerTool("aws_list_iam_users", {
...
Returns a markdown table of users, their access keys, key status, and last-used info.`,
        ...
        const keysCmds = users
            .map((u) => `echo "USER:${u.UserName}" && aws iam list-access-keys --user-name "${u.UserName}" --output json`)
            .join(" && ");
        ...
        for (const user of users) {
            ...
            if (keyRows.length === 0) {
                ...
                for (const key of keyRows) {
                    ...
                    lines.push(`| \`${user.UserName}\` | ${created} | \`${key.AccessKeyId}\` | ${status} | — |`);
                }
            }
        }
        let text = lines.join("\n");
    ...
    server.registerTool("aws_check_iam_permissions", {
...
Useful to check if omv-main-cli (or another user) has a specific permission before running a task.`,
        ...
        const cmd = `aws iam simulate-principal-policy` +
        ...
        if (evals.length === 0) {
            ...
            for (const ev of evals) {
                ...
                lines.push(`**Decision:** ${allowed ? "✅ ALLOWED" : "❌ DENIED"} (\`${ev.EvalDecision}\`)`);
            }
        }
        return { content: [{ type: "text", text: lines.join("\n") }] };
    });
    // ── aws_list_acm_certs ────────────────────────────────────────────────────
    server.registerTool("aws_list_acm_certs", {
...
Useful to audit SSL/TLS certificates managed in AWS.`,
        ...
        const describeCmd = certs
            .map((c) => `echo "CERT:${c.CertificateArn}" && aws acm describe-certificate --certificate-arn "${c.CertificateArn}" --region us-east-1 --output json`)
            .join(" && ");
        ...
        for (const cert of certs) {
            ...
            lines.push(`| \`${domain}\` | ${status} | ${expiry} | \`...${arnShort}\` | ${sans} |`);
        }
        let text = lines.join("\n");
        if (text.length > CHARACTER_LIMIT) {
            text = text.slice(0, CHARACTER_LIMIT) + "\n\n...(truncated)";
    ...
    server.registerTool("aws_grant_route53_delete_health_check", {
...
Run aws_check_iam_permissions first to verify the user currently lacks this permission.`,
        ...
        const cmd = `aws iam put-user-policy` +
    ...
    server.registerTool("aws_revoke_route53_delete_health_check", {
...
Runs aws iam delete-user-policy on the Pi.`,
        ...
        const cmd = `aws iam delete-user-policy` +
            ` --user-name omv-main-cli` +
            ` --policy-name route53-delete-hc`;
        const r = await runOnNode("omv-main", cmd);
        if (r.error) {
    ...
    // ── aws_delete_acm_cert ───────────────────────────────────────────────────
    server.registerTool("aws_delete_acm_cert", {
...
Use aws_list_acm_certs to find the ARN first.`,
        inputSchema: z.object({
            certificate_arn: z
                .string()
                .min(1)
                .describe("Full ARN of the ACM certificate to delete, e.g. arn:aws:acm:us-east-1:..."),
        }),
        annotations: { readOnlyHint: false, destructiveHint: true },
