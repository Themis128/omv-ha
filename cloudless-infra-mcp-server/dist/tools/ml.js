...
export function registerMlTools(server) {
    // ── ml_pipeline_status ────────────────────────────────────────────────────
    server.registerTool("ml_pipeline_status", {
...
Use this to diagnose pipeline failures or verify a successful weekly training run.`,
        inputSchema: z.object({
            job: CronJobSchema.optional().describe("Filter to a specific CronJob (omit for all 7 jobs)"),
        }),
        annotations: { readOnlyHint: true, destructiveHint: false },
    }, async ({ job }) => {
        const selector = job ? `-l job-name=${job}` : `-l app=ml-pipeline`;
        const cmd = `${KUBECTL} get cronjobs -n ${NS} -l app=ml-pipeline` +
            ` && echo '--- Recent Jobs ---'` +
    ...
    // ── ml_trigger_job ────────────────────────────────────────────────────────
    server.registerTool("ml_trigger_job", {
...
⚠️  concurrencyPolicy: Forbid — if a job is already running for this CronJob, the new one will pend until it completes.`,
        ...
        const cmd = `${KUBECTL} create job ${jobName} --from=cronjob/${job} -n ${NS}` +
            ` && echo '--- Job created ---'` +
    ...
    // ── ml_get_logs ───────────────────────────────────────────────────────────
    server.registerTool("ml_get_logs", {
...
Fetches the last 200 lines by default.`,
        ...
        const cmd = `POD=$(${KUBECTL} get pods -n ${NS} -l job-name --sort-by=.metadata.creationTimestamp ` +
    ...
    // ── ml_run_history ────────────────────────────────────────────────────────
    server.registerTool("ml_run_history", {
...
Requires at least one successful ml-feature-engineer + training run to have data.`,
        ...
        const sql = `SELECT model_name, trained_at, training_rows, is_champion, ` +
    ...
    // ── ml_get_scores ─────────────────────────────────────────────────────────
    server.registerTool("ml_get_scores", {
        title: "Get ML Scores",
...
Returns the first N rows with all columns.`,
    ...
    // ── ml_check_models ───────────────────────────────────────────────────────
    server.registerTool("ml_check_models", {
...
Use this to confirm a training job successfully saved its model artifact.`,
        ...
        const cmd = `AWS_DEFAULT_REGION=us-east-1 aws s3 ls s3://${S3_BUCKET}/${prefix} --recursive` +
            ` && echo '--- Parquet scores ---'` +
    ...
    // ── ml_anomaly_latest ─────────────────────────────────────────────────────
    server.registerTool("ml_anomaly_latest", {
...
High anomaly_score (closer to 0 or positive) = more anomalous; threshold is typically around -0.15.`,
        ...
        const sql = `SELECT window_start, is_anomaly, anomaly_score, request_count, error_rate, p95_ms, scored_at ` +
            `FROM anomaly_flags ` +
    ...
    // ── ml_feature_summary ────────────────────────────────────────────────────
    server.registerTool("ml_feature_summary", {
...
Use to verify the ml-feature-engineer job ran successfully before triggering training jobs.`,
        ...
        const sql = `SELECT feature_name, computed_at, row_count, s3_parquet_path ` +
