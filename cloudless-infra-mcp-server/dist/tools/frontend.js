...
export function registerFrontendTools(server) {
    ...
    server.registerTool("frontend_check_navbar", {
...
Returns a pass/fail table with occurrence counts. Run after any navbar change.`,
    ...
    // ── frontend_check_pwa ────────────────────────────────────────────────────
    server.registerTool("frontend_check_pwa", {
...
- "live"       — curls the live /api/pwa-manifest endpoint (requires live_url)`,
        inputSchema: z.object({
            ...
            live_url: z
                .string()
                .optional()
                .describe('Base URL for live check, e.g. "https://cloudless.gr". Required when source=live.'),
        }),
        annotations: { readOnlyHint: true, destructiveHint: false },
    ...
    server.registerTool("frontend_deploy_cloudless_gr", {
