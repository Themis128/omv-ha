...
import { CLOUDFLARE_API_TOKEN, CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_ZONE_ID, CHARACTER_LIMIT, } from "../constants.js";
...
async function cfRawFetch(url, options = {}) {
...
let _resolvedZoneId = null;
async function resolveZoneId() {
    if (CLOUDFLARE_ZONE_ID)
        return CLOUDFLARE_ZONE_ID;
    if (_resolvedZoneId)
        return _resolvedZoneId;
    const data = (await cfRawFetch(`${CF_API}/zones?name=cloudless.gr&status=active`));
    if (!data.success || !data.result.length) {
        throw new Error(`Cannot auto-discover zone ID for cloudless.gr: ${cfError(data)}. ` +
...
async function cfAccountFetch(path, options = {}) {
...
export function registerCloudflareTools(server) {
    ...
    server.registerTool("cloudflare_verify_token", {
...
Run this first to confirm the token is working before other Cloudflare tools.`,
        ...
        try {
            const data = (await cfApiFetch("/user/tokens/verify"));
            if (!data.success) {
                return {
            ...
            try {
                ...
                zoneNote = `\nZone ID: ⚠️ could not resolve — ${e.message}`;
            }
            return {
                content: [
                    {
    ...
    server.registerTool("cloudflare_get_zone_id", {
...
Returns zone ID, name server addresses, status, and plan.`,
        ...
        try {
            const data = (await cfApiFetch(`/zones?name=${encodeURIComponent(name)}`));
            if (!data.success)
...
Always run this before adding or deleting records to check current state.`,
        ...
        try {
            ...
            const data = (await cfFetch(`/dns_records?${params}`));
            if (!data.success)
                return { content: [{ type: "text", text: `❌ ${cfError(data)}` }] };
            if (!data.result.length)
...
Use cloudflare_list_dns_records first to get the record ID.`,
    ...
    server.registerTool("cloudflare_bulk_restore_dns", {
  ...
  { "type": "MX",    "name": "@",                "content": "mail.example.com", "priority": 10, "proxied": false }
        ...
        for (const rec of records) {
            try {
                ...
                results.push(`❌ ${rec.type.padEnd(6)} ${rec.name}: ${e.message}`);
            }
        }
        ...
        return {
            content: [
                {
                    type: "text",
                    text: trunc(`## DNS Restore: ${ok} created, ${skipped} skipped, ${failed} failed\n\n${results.join("\n")}`),
                },
            ],
        };
    });
...
Filter by scope: "zone", "account", "user", or a keyword like "dns" or "load".`,
        ...
        try {
            ...
            const rows = groups.map((g) => `${g.id}  ${g.name.padEnd(50)}  [${g.scopes.join(", ")}]`);
            return {
                content: [
                    {
                        type: "text",
...
All zones: "com.cloudflare.api.account.zone.*"`,
    ...
    server.registerTool("cloudflare_rotate_token", {
...
4. Optionally deletes the old token (set delete_old=false to keep it during transition)`,
        ...
        try {
            ...
            const listData = (await cfApiFetch("/user/tokens"));
            if (!listData.success)
            ...
            const detailData = (await cfApiFetch(`/user/tokens/${oldToken.id}`));
            ...
            if (expires_on)
                createBody.expires_on = expires_on;
...
Requires "User:User API Tokens:Edit" permission.`,
    ...
    server.registerTool("cloudflare_list_lb_monitors", {
...
Returns monitor ID, type, path, interval, and expected status codes.`,
        ...
        try {
            const data = (await cfAccountFetch("/load_balancers/monitors"));
            if (!data.success)
                return { content: [{ type: "text", text: `❌ ${cfError(data)}` }] };
            if (!data.result.length)
    ...
    server.registerTool("cloudflare_list_lb_pools", {
...
Returns pool ID, name, origins, health, and monitor association.`,
        ...
        try {
            const data = (await cfAccountFetch("/load_balancers/pools"));
            if (!data.success)
                return { content: [{ type: "text", text: `❌ ${cfError(data)}` }] };
            if (!data.result.length)
            ...
            const rows = data.result.map((p) => {
                const origins = p.origins
                    .map((o) => `    ${o.enabled ? "✅" : "⏸"} ${o.name.padEnd(20)} ${o.address}  w=${o.weight}  healthy=${o.healthy}`)
                    .join("\n");
                return [
                    `ID:      ${p.id}`,
                    `Name:    ${p.name}`,
    ...
    server.registerTool("cloudflare_list_load_balancers", {
...
Returns LB name, hostname, pool order, proxied status, and session affinity.`,
        ...
        try {
            const data = (await cfFetch("/load_balancers"));
            if (!data.success)
                return { content: [{ type: "text", text: `❌ ${cfError(data)}` }] };
            if (!data.result.length)
    ...
    server.registerTool("cloudflare_update_lb_pool_origins", {
...
Use cloudflare_list_lb_pools to get pool IDs.`,
    ...
    server.registerTool("cloudflare_provision_lb", {
...
- Active-passive: primary pool weight=1, secondary pool weight=0 (failover)`,
        ...
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true },
    }, async ({ hostname, primary_origin, primary_origin_port, secondary_origin, secondary_origin_port, health_check_path, dry_run, }) => {
        if (dry_run) {
            return {
                content: [
                    {
        ...
        try {
            ...
            steps.push(`✅ Secondary pool created: ${secondaryPoolData.result.name} (${secondaryPoolId})`);
    ...
    server.registerTool("cloudflare_bootstrap_two_tokens", {
...
Requires:
  ...
  - gh CLI authenticated locally (to set GitHub secret + variable)`,
        ...
        const globalFetch = async (path, options = {}) => {
        ...
        try {
            ...
            const zoneData = (await globalFetch("/zones?name=cloudless.gr&status=active"));
            ...
            const pgData = (await globalFetch("/user/tokens/permission_groups"));
            if (!pgData.success) {
                return { content: [{ type: "text", text: `❌ Cannot fetch permission groups: ${cfError(pgData)}` }] };
            }
            const pg = pgData.result;
            const findPg = (name) => pg.find((g) => g.name === name);
