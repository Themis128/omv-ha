...
slash command but as a structured MCP tool.`,
      inputSchema: z.object({
        ...
        limit: z
          ...
          .default(100)
          .describe(
            "How many recent runs to scan to determine per-workflow status. Default 100.",
          ),
      }),
    ...
    async ({ repo, limit }) => {
      ...
      try {
        ...
        const onlineCount = runnersData.runners.filter(
          (r) => r.status === "online",
        ).length;
        ...
        if (billingLock) {
          verdict = "CRITICAL";
          issues.push(
            "⚡ **Billing lock** — GitHub-hosted runners disabled. Resolve at github.com/settings/billing.",
          );
        }
...
Use after gh_ci_summary or gh_pr_checks identifies a failing run ID.`,
      inputSchema: z.object({
        ...
        run_id: z
          ...
          .int()
          .describe(
            "Workflow run ID (from gh_ci_summary or gh_workflow_list).",
          ),
        max_lines: z
          .number()
          .int()
          ...
          content: [
            {
              type: "text",
              text:
                `## Failure logs — ${meta.workflowName} run ${run_id}\n` +
...
environment/race-condition issue (flaky).`,
      inputSchema: z.object({
        ...
        workflow: z
          .string()
          .describe(
            'Workflow filename to analyse (e.g. "deploy-pi.yml", "e2e.yml").',
          ),
        limit: z
          .number()
          .int()
    ...
    async ({ repo, workflow, limit }) => {
      ...
      try {
        ...
        let currentStreakType =
          completed[0].conclusion === "success" ? "success" : "failure";
        let maxFailStreak = 0;
          ...
          content: [
            {
              type: "text",
              text:
                `## ${verdictIcon} Flaky Analysis — \`${workflow}\` on ${fullRepo}\n\n` +
                `**Verdict:** ${verdict}\n` +
