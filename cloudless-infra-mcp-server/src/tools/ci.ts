    ...
    async ({ repo, limit }) => {
      ...
      try {
        ...
        const runs = await ghJson<Array<{
          ...
          url: string;
          event: string;
        }>>(["run", "list", "--repo", fullRepo, "--limit", String(limit),
        ...
        const byWorkflow = new Map<string, typeof runs[0]>();
        for (const r of runs) {
          if (!byWorkflow.has(r.workflowName)) byWorkflow.set(r.workflowName, r);
        }
        ...
        if (firstFail) {
          try {
            const jobs = await ghJson<{ jobs: Array<{
              started_at: string; completed_at: string;
              steps: unknown[]; conclusion: string;
            }> }>(["api", `repos/${fullRepo}/actions/runs/${firstFail.databaseId}/jobs`]);
        ...
        const runnersData = await ghJson<{ total_count: number; runners: Array<{
          name: string; status: string; busy: boolean;
        ...
        if (fail.length > 0 && verdict !== "CRITICAL") verdict = "DEGRADED";

        const verdictIcon = verdict === "HEALTHY" ? "✅" : verdict === "DEGRADED" ? "⚠️" : "🛑";
        const parts: string[] = [`## ${verdictIcon} CI Dashboard — ${fullRepo} — ${verdict}`];

        if (issues.length > 0) parts.push("\n**Critical issues:**\n" + issues.join("\n"));

        parts.push(`\n**Runners:** ${onlineCount}/${runnersData.total_count} online, ${busyCount} busy`);

        if (fail.length > 0) parts.push(`\n### ❌ FAILING (${fail.length})\n${fail.join("\n")}`);
        if (running.length > 0) parts.push(`\n### ⏳ RUNNING (${running.length})\n${running.join("\n")}`);
        if (ok.length > 0) parts.push(`\n### ✅ OK (${ok.length})\n${ok.join("\n")}`);
...
failing, pending, or skipped.`,
    ...
    async ({ repo, pr }) => {
      ...
      try {
        ...
        }>(["pr", "view", String(pr), "--repo", fullRepo,
    ...
    async ({ repo, run_id, max_lines }) => {
      ...
      try {
        ...
        const meta = await ghJson<{
          ...
          createdAt: string;
          url: string;
        }>(["run", "view", String(run_id), "--repo", fullRepo,
    ...
    async ({ repo, workflow, limit }) => {
      ...
      try {
        const runs = await ghJson<Array<{
          ...
          headBranch: string;
          displayTitle: string;
        }>>(["run", "list", "--repo", fullRepo, "--workflow", workflow,
          "--limit", String(limit),
        ...
        for (const r of completed) {
          ...
          if (isSuccess) { tempSuccess++; tempFail = 0; }
          else { tempFail++; tempSuccess = 0; }
        ...
        if (successRate >= 85) {
          verdict = "STABLE"; verdictIcon = "✅";
        } else if (successRate < 15) {
          verdict = "CONSISTENTLY_FAILING"; verdictIcon = "🛑";
        } else {
          verdict = "FLAKY"; verdictIcon = "⚠️";
        }
...
identify when a regression was introduced.`,
    ...
    async ({ repo, workflow }) => {
      ...
      try {
        ...
        const parts: string[] = [`## 🚀 Deployment Status — ${fullRepo} / \`${workflow}\``];
