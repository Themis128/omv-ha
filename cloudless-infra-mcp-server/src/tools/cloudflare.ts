...
Useful to inspect current state before adding or deleting records.`,
    ...
    async ({ type, name }) => {
      ...
      const data = (await cfFetch(`/dns_records${query}`)) as {
        success: boolean;
        result: Array<{
          id: string;
          type: string;
      ...
      const rows = data.result.map((r) =>
        `${r.id}  ${r.type.padEnd(6)}  ${r.name.padEnd(35)}  ${r.content}  TTL=${r.ttl}  proxied=${r.proxied}`,
      );
...
For CNAME tunnel records use content="<tunnel-id>.cfargotunnel.com" and proxied=true.`,
      inputSchema: z.object({
        ...
        name: z
          .string()
          .describe(
            'Record name — use "@" for apex, or e.g. "staging", "*.cloudless.online"',
          ),
        content: z
          .string()
...
Tunnel recovers automatically (http2 protocol, stable behind CGNAT).`,
    ...
    async () => {
      const cmd =
        `sudo systemctl restart cloudflared && sleep 3 && systemctl is-active cloudflared`;
